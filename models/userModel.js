...
const findByEmail = async (email) => {
  const usuarios = await db().then((data) =>
    data.getTable('users').select().where('email = :email').bind('email', email).execute(),
  );
...
const findById = async (id) => {
  const usuarios = await db().then((data) =>
    data.getTable('users').select().where('id = :id').bind('id', id).execute(),
  );
  const user = usuarios.fetchAll();
