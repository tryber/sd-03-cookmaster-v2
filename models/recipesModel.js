...
const findRecipById = async (id) => {
  const receitas = await db().then((data) =>
    data.getTable('recipes').select().where('id = :id').bind('id', id).execute(),
  );
...
const deleteRecipe = async (id) =>
  db().then((db2) => db2.getTable('recipes').delete().where('id = :id').bind('id', id).execute());
